...
return array(
	...
	'smwgElasticsearchConfig' => [
		...
		'query' => [
			...
			// Size (limit) of the subquery construct which executed as separated
			// search request.
			'subquery.size' => 5000,
			...
			// as it directly impacts performance.
			//
			// @see https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-terms-query.html
			'subquery.terms.lookup.result.size.index.write.threshold' => 100,

			// Intermediary optimization to allow a subquery (executed as part of
			// a "source" query and uses the special lookup index) to be reused
			// (aka cached) for any succeeding query executions for the set time
			// frame and hereby avoids fetching subquery results on repeated
			// requests especially for when the source query changes its limit
			// and offset parameters.
			//
			// Only subqueries with `subquery.terms.lookup.result.size.index.write.threshold`
			// will be available as index cache.
			//
			...
			// Fields that comprise a wide proximity search with some fields being
			...
			// of the text fields.
			//
			// This means that [[in:Foo]] with titles matching `Foo` will be
			// assigned a 5 times higher score and therefore appears higher in a
